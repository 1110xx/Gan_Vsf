...
def train_loop_hybrid(encoder, decoder, discriminator, train_loader, val_loader, args):
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        inject_gate = torch.sigmoid(encoder.inject_gate).item()
        print(f"inject_gate={inject_gate:.4f}, num_slots={encoder.num_slots}")
        # 保存最佳模型
...
def main():
    ...
    parser.add_argument('--temporal_dilations', type=int, nargs='+', default=[1, 2, 4, 8],
                        help='时序膨胀率列表')
    parser.add_argument('--node_emb_dropout', type=float, default=0.5, help='节点 embedding dropout 比例，强迫利用观察上下文')
